...
class DataUtility():
    ...
    def load_data(self, filename, selected_genes=None):
        '''
        load csv file
        Args:
            filename ([str]): filename
            selected_genes ([list], optional): [description]. Defaults to None.

        Returns:
            x ([dataframe]): dataframe from filename
            response ([series]): matched response of filename
            samples ([Index]): sample list of filename
        ...
        if 'response' in self.cached_data:
            ...
            # TODO data loading
            labels = self.get_response()
            self.cached_data['response'] = labels

        # join with the labels
        # using the id to join two dataframes, which lets the data is in the same order of the response
    ...
    def load_TMB(self, filename):
        x, response, samples, genes = self.load_data(filename)
        x = np.sum(x, axis=1)
        x = np.array(x)
        x = np.log(1. + x)
        n = x.shape[0]
    ...
    def load_CNV_burden(self, filename):
        x, response, samples, genes = self.load_data(filename)
        x = np.sum(x, axis=1)
        x = np.array(x)
        x = np.log(1. + x)
    ...
    def load_data_type(self, data_type='gene', cnv_levels=5, cnv_filter_single_event=True, mut_binary=False, selected_genes=None):
        '''
        Get corresponding data.
        Args:
            data_type (str, optional): data type. Defaults to 'gene'.
            cnv_levels (int, optional): . Defaults to 5.
            cnv_filter_single_event (bool, optional): [description]. Defaults to True.
            mut_binary (bool, optional): If true, all the values over 1 will be changed to 1. Defaults to False.
            selected_genes (list, optional): list of the selected genes. Defaults to None.
        ...
        if data_type == 'TMB':
            ...
            '''
            x, response, info, genes = self.load_TMB('P1000_final_analysis_set_cross_important_only.csv')
        ...
        if data_type == 'mut_important_plus_hotspots':
            x, response, info, genes = self.load_data('P1000_final_analysis_set_cross_important_only_plus_hotspots.csv', 
                                                      selected_genes)

        if data_type == 'mut_hotspots':
            x, response, info, genes = self.load_data('P1000_final_analysis_set_cross_hotspots.csv', selected_genes)
        ...
        if data_type == 'gene_final_no_silent':
            x, response, info, genes = self.load_data('P1000_final_analysis_set_cross__no_silent.csv', selected_genes)
        if data_type == 'cnv':
        ...
        if data_type == 'gene_expression':
            x, response, info, genes = self.load_data('P1000_adjusted_TPM.csv', selected_genes)

        if data_type == 'fusions':
            x, response, info, genes = self.load_data('p1000_onco_ets_fusions.csv', None)

        if data_type == 'cnv_burden':
            x, response, info, genes = self.load_data('P1000_data_CNA_burden.csv', None)
            # x.loc[:, :] = 0.

        if data_type == 'fusion_genes':
            x, response, info, genes = self.load_data('fusion_genes.csv', selected_genes)
            # x.loc[:,:]=0.
    ...
    def combine(self, x_list, y_list, rows_list, cols_list, data_type_list, combine_type, use_coding_genes_only=False):
        # cols are the genes
        ...
        if use_coding_genes_only:
            f = join(self.data_path, 'genes/HUGO_genes/protein-coding_gene_with_coordinate_minimal.txt')
        ...
        for x, y, r, c in zip(x_list, y_list, rows_list, cols_list):
            ...
            # let the df have the same column as all the genes
    ...
    def __init__(self, data_dir, data_type='mut', account_for_data_type=None, cnv_levels=5,
        ...
        if selected_genes is not None:
            if type(selected_genes) == list:
                ...
                selected_genes_file = join(self.data_path, 'genes')
        ...
        if type(data_type) == list:
            ...
            for t in data_type:
                '''
                x: dataframe of the loading data; y: [response], series, matched response of loading data
                rows: [samples], sample list of filename, 
                cols: [genes], the intersection of all the genes in the loading data and selected genes
                '''
                x, y, rows, cols = data_utility.load_data_type(t, cnv_levels, cnv_filter_single_event, mut_binary, selected_genes)
                x_list.append(x), y_list.append(y), rows_list.append(rows), cols_list.append(cols)
            
            x, y, rows, cols = data_utility.combine(x_list, y_list, rows_list, cols_list, data_type, combine_type,
                                                    use_coding_genes_only)
            x = pd.DataFrame(x, columns=cols)

        else:
            x, y, rows, cols = data_utility.load_data_type(data_type, cnv_levels, cnv_filter_single_event, mut_binary,
                                                           selected_genes)
        ...
        if account_for_data_type is not None:
            ...
            for t in account_for_data_type:
                x_, y_, rows_, cols_ = data_utility.load_data_type(t, cnv_levels, cnv_filter_single_event, mut_binary,
                                                                   selected_genes)
        ...
        if selected_samples is not None:
            selected_samples_file = join(self.processed_path, selected_samples)
